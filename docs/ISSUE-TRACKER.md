...
### #008 — MiniMax Codex Responses 本地路由名与旧运行时不兼容

| 字段 | 值 |
|------|-----|
| **标题** | MiniMax Models 走 Responses API 返回 404 |
| **状态** | 🟢 已修复 |
| **优先级** | P1 |
| **发现日期** | 2026-09-02 |
| **修复日期** | 2026-09-02 |
| **发现人** | 用户反馈 |
| **影响范围** | Codex CLI + MiniMax-M2.7 / MiniMax-M3 |
| **现象** | 8090 旧运行时对 `/responses` 直接返回 404；用当前源码复测时，本地路由名 `minimax3-c` 又被原样发给 MiniMax，上游返回 `unknown model` |
| **根因** | 两层问题：8090 正在运行的进程仍是旧代码，不支持新增 `/responses` 别名；当前源码虽支持该别名，但透传时没有把本地 `name`（`minimax3-c`）映射回上游 `model_id`（`MiniMax-M3`）。配置中的 `https://api.minimax.cn` 本身正确 |
| **修复提交** | 未提交（透传端点拼接 + 本地模型名映射待入库） |
| **开发记录** | docs/dev-notes/2026-09-02-minimax-responses-routing.md |
| **回归测试** | 8096 测试端口使用 `minimax3-c` 请求 `/responses` 返回 200 且上游 model 为 `MiniMax-M3`；透传单测 PASS；核心模块 py_compile PASS；8090 需重启加载修复 |


---

### #009 — GUI 配置页落后于后端新增字段

| 字段 | 值 |
|------|-----|
| **标题** | GUI 保存配置会丢失 tool_result_visible_fallback 等新字段 |
| **状态** | 🟢 已修复 |
| **优先级** | P2 |
| **发现日期** | 2026-09-02 |
| **发现人** | 用户反馈 |
| **影响范围** | PyQt GUI 配置保存；新增后端运行时开关 |
| **现象** | 后端已加入全局 `tool_result_visible_fallback`，但 GUI 无对应开关；现有 `apply_model` 直接构造有限字段的对象，保存/应用时容易让未暴露字段回到默认值 |
| **根因** | 前端配置页未同步最新配置模型 |
| **修复日期** | 2026-09-02 |
| **修复提交** | b6b8b28 |
| **开发记录** | docs/dev-notes/2026-09-02-gui-backend-config-parity.md |
| **回归测试** | PyQt GUI 回归用例 PASS；核心模块导入 PASS；py_compile PASS |


---

### #010 — glm-5.3-flash 图片输入未按多模态模型转发

| 字段 | 值 |
|------|-----|
| **标题** | glm-5.3-flash 支持多模态但转换器把图片降级或丢失 |
| **状态** | 🟢 已修复 |
| **优先级** | P1 |
| **发现日期** | 2026-09-02 |
| **发现人** | 用户反馈 |
