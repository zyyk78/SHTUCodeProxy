...
```text
SHTUCodeProxy-v4.3.3-windows-x64.exe
```
...
```text
SHTUCodeProxy-v4.3.3-windows-x64.zip
```
...
```text
SHTUCodeProxy-v4.3.3-linux-x86_64
SHTUCodeProxy-v4.3.3-linux-x86_64-python-launcher.tar.xz
shtucodeproxyctl-v4.3.3-linux-x86_64
SHTUCodeProxy-v4.3.3-linux-x86_64-headless-cli.zip
```

For the single-file binary, make it executable and run it from a desktop session:

```bash
chmod +x SHTUCodeProxy-v4.3.3-linux-x86_64
./SHTUCodeProxy-v4.3.3-linux-x86_64
```
...
```bash
tar -xf SHTUCodeProxy-v4.3.3-linux-x86_64-python-launcher.tar.xz
cd SHTUCodeProxy-v4.3.3-linux-x86_64-python-launcher
...
- On headless servers, use `SHTUCodeProxy-v4.3.3-linux-x86_64-headless-cli.zip`. It contains the no-GUI executable, `headless-config.example.json`, and an editable `config.json` copy.
...
| Input Types | Whether this model route supports image, audio, and video input | Images enabled for GPT-5.5 and qwen-instruct |

> **Important:** For GPT-5 and newer GPT models, set `API Format` to `responses`.
> If an input type is disabled and a client sends that type, SHTUCodeProxy returns a normal assistant message explaining that the selected model does not support it instead of forwarding the request upstream. File/document inputs keep the existing passthrough behavior and are not controlled by these switches.
...
`Default streaming` is enabled by default. It only applies when a client request omits the `stream` field; explicit `stream=true` and `stream=false` requests are always respected. For GPT-5.5 and other reasoning-heavy Responses models, keep enough `max_output_tokens` for both reasoning and visible output, especially for image input.
...
```text
release\SHTUCodeProxy-v4.3.3-windows-x64.exe
release\SHTUCodeProxy-v4.3.3-windows-x64.zip
dist\SHTUCodeProxy\SHTUCodeProxy.exe
```

Use `release\SHTUCodeProxy-v4.3.3-windows-x64.exe` for normal Windows users. It is a single-file executable and does not require Python or the `_internal` folder.

Use `release\SHTUCodeProxy-v4.3.3-windows-x64.zip` as the Windows portable folder package. If you distribute the zip package, users must extract the whole folder because the `_internal` runtime folder is required by the folder build.
...
```bash
unzip SHTUCodeProxy-v4.3.3-source-linux-macos.zip
cd SHTUCodeProxy-v4.3.3-source
...
```bash
unzip SHTUCodeProxy-v4.3.3-linux-x86_64-headless-cli.zip
cd SHTUCodeProxy-v4.3.3-linux-x86_64-headless-cli
chmod +x shtucodeproxyctl-v4.3.3-linux-x86_64
nano config.json
./shtucodeproxyctl-v4.3.3-linux-x86_64 apply-config config.json --write-claude --write-codex --start
./shtucodeproxyctl-v4.3.3-linux-x86_64 status
```
...
- `models`: defines all available routes. `model_id` is the local name clients request; `upstream_model` is the real provider model name; `api_format` is `responses` or `chat_completions`; `supports_image`, `supports_audio`, and `supports_video` control image/audio/video inputs for that route.
...
- `default_stream`: when `true`, requests that omit `stream` default to streaming. Explicit request values still win.
