...
```text
SHTUCodeProxy-v4.4.2-windows-x64.exe
```
...
```text
SHTUCodeProxy-v4.4.2-windows-x64.zip
```
...
```text
SHTUCodeProxy-v4.4.2-linux-x86_64
SHTUCodeProxy-v4.4.2-linux-x86_64-python-launcher.tar.xz
shtucodeproxyctl-v4.4.2-linux-x86_64
SHTUCodeProxy-v4.4.2-linux-x86_64-headless-cli.zip
```

For the single-file binary, make it executable and run it from a desktop session:

```bash
chmod +x SHTUCodeProxy-v4.4.2-linux-x86_64
./SHTUCodeProxy-v4.4.2-linux-x86_64
```
...
```bash
tar -xf SHTUCodeProxy-v4.4.2-linux-x86_64-python-launcher.tar.xz
cd SHTUCodeProxy-v4.4.2-linux-x86_64-python-launcher
...
- On headless servers, use `SHTUCodeProxy-v4.4.2-linux-x86_64-headless-cli.zip`. It contains the no-GUI executable, `headless-config.example.json`, and an editable `config.json` copy.
...
```text
release\SHTUCodeProxy-v4.4.2-windows-x64.exe
release\SHTUCodeProxy-v4.4.2-windows-x64.zip
dist\SHTUCodeProxy\SHTUCodeProxy.exe
```

Use `release\SHTUCodeProxy-v4.4.2-windows-x64.exe` for normal Windows users. It is a single-file executable and does not require Python or the `_internal` folder.

Use `release\SHTUCodeProxy-v4.4.2-windows-x64.zip` as the Windows portable folder package. If you distribute the zip package, users must extract the whole folder because the `_internal` runtime folder is required by the folder build.
...
```bash
unzip SHTUCodeProxy-v4.4.2-source-linux-macos.zip
cd SHTUCodeProxy-v4.4.2-source
...
```bash
unzip SHTUCodeProxy-v4.4.2-linux-x86_64-headless-cli.zip
cd SHTUCodeProxy-v4.4.2-linux-x86_64-headless-cli
chmod +x shtucodeproxyctl-v4.4.2-linux-x86_64
nano config.json
./shtucodeproxyctl-v4.4.2-linux-x86_64 apply-config config.json --write-claude --write-codex --start
./shtucodeproxyctl-v4.4.2-linux-x86_64 status
```
