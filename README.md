# SHTUCodeProxy 使用说明书
**本项目修改自SaberJack的项目，目前作为一个独立的分支维护**  
**本项目在原有项目的基础上进行了部分代码重构，包括代码拆分，添加透传模式（即可以使用非SHTU的A社接口），添加了一些安全相关的配置（虽然没啥用），优化了部分代码性能，添加了一些日志控制功能，修复了一些小bug**  
**该分支可能会与主分支越来越远**  
**目前我仅修改了后端（/src中）的相关代码，前端则未进行更改，也没有进行打包，请现在源代码后，直接修改其中的config文件并运行proxy，随后修改claude code的settings即可**  
目前未测试项目：
>图片、音频相关  
>CodeX适配  
>部分网络安全相关

注以下说明由AI自动生成，部分配置选项仅供参考！
---
